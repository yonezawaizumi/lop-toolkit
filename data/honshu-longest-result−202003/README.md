...
* 最後に、以下のとおり、経路特定区間を考慮する
    ...
    1. 2.のうちの最長のものが「真のJR最長O型きっぷのルート」となる
...
* 「当該区間を通過する場合、最長経路の計算が必ず経路特定区間を破ってしまう」経路は、あらかじめデータから除外する
    ...
    * これを満たす経路特定区間は以下のものであり、うち、本州内のものは1.以外となる
        ...
        1. 三原-海田市 →呉線を除外
        1. 岩国-櫛ケ浜 →山陽本線を除外
* 前項以外の経路については、旅客営業取扱基準規程第109条が「適用されれば経路どおり」というものであるから、行き止まり枝線以外の分岐を含む経路は除外せず、すべてそのまま残し、さらに前述の考え方で「経路特定区間で長い方を通過することを禁止する」制約式を追加する。
* それらの計算の結果を踏まえ、すべての経路特定区間に対し、「その区間内発着により経路特定区間の適用を回避できる」計算を行う。
...
なお、これらのうち、8号（山陽・呉線）および9号（山陽・岩徳）については、計算上の経路から「長い方」を除外してあるため、以下の計算結果に上記の差分を加えたものが「区間内発着の運賃計算キロ」となることに留意してください。
...
そしてこれらのうち、呉線および岩徳線は、計算用の経路として短い方のみを残しての計算結果ですので、経路特定区間内の駅で発着にした場合に「稼げる」差分を加算します。その結果は、以下のとおりです。
